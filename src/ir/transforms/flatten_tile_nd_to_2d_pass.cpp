...
BatchPageResult ExtractBatchPage(const BatchOperandInfo& info, const std::vector<int64_t>& operand_dims,
  ...
  if (is_mat_load && original_load_offsets && original_load_input_type &&
    ...
    // Keep ND offsets and shapes on the load call so codegen still sees the
    // original ND source window. type inference will still produce an ND
    // TileType from these ND shapes; this pass immediately overrides the call
    // result type to 2D below because hardware tiles are always 2D.
    // For transposed loads (transpose=True), ND offsets are required so that
    // codegen routes the load through the standard DN path. A flat 2D ND view
    // cannot represent within-batch column-major addressing.
    ...
    // DeduceTileLoadType produces ND TileType from ND shapes, but hardware tiles
    // are always 2D. Manually override the result type to 2D.
...
BatchMatmulResult LowerBatchMatmul(const AssignStmtPtr& assign, const CallPtr& call,
  ...
  if (direct_store.detected) {
    ...
    if (store_tensor_type->shape_.size() > 2) {
      // Build ND partition shape: [1, ..., 1, M, N] (left-padded with 1s for batch dims)
  ...
  for (int64_t i = 0; i < batch_count; ++i) {
    ...
    if (direct_store.detected) {
      ...
      // Keep ND offsets — codegen uses ND tensor_view + ND partition_view.
...
std::vector<StmtPtr> TransformBody(const std::vector<StmtPtr>& stmts, FlattenContext& ctx,
  ...
  for (size_t stmt_index = 0; stmt_index < stmts.size(); ++stmt_index) {
    ...
    // ---- tile.load on >2D tile: preserve ND source window, but flatten the result tile ----
    // Keep ND offsets/shapes on the call for codegen, then manually replace the
    // inferred ND TileType with a 2D TileType because hardware tiles are always 2D.
    if (op_name == "tile.load") {
      ...
      if (result_tile && result_tile->shape_.size() > 2) {
        // ND tile.load: keep ND offsets/shapes, but construct 2D TileType for the result.
        // DeduceTileLoadType produces ND TileType from ND shapes, but hardware tiles
        // are always 2D. The pass manually overrides the result type to 2D.
    ...
    // ---- tile.store: inject ND partition shape for ND tensors ----
    // tile.store semantics: (2D) tile → ND tensor. ND offsets are preserved;
    // codegen uses ND tensor_view + ND partition_view to produce 2D result.
    ...
    if (op_name == "tile.store") {
      ...
      if (orig_tile_type && out_tensor_type && out_tensor_type->shape_.size() > 2) {
        // Inject ND partition shape tuple as 4th argument.
 ...
 * 2. Preserving ND offsets/shapes in tile.load/store for codegen to use
 *    with ND tensor_view + ND partition_view
 */
